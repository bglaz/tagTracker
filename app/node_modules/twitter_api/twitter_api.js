...
Twitter.prototype = {
	getTweets: 	function(tag,callback) {
		var requestOptions = this.getRequestOpts(tag);
		...
		var req = https.request(requestOptions,function(res) {
			...
			res.on('data',function(data) {
				...
				//highlight instances of tag in tweet text
				if(json.text) {
					var regex = new RegExp("(\\s|^|#)(" + tag + ")(\\s|[.,?!-]+|$)","ig");
					json.text = json.text.replace(regex,"$1<b>$2</b>$3");
				}
				callback(json);
			});
		});
		...
		req.on('error',function(e) {
			console.log("Problem with tweet request: " + e.message);
		});

		req.write("track=" + encodeURIComponent(tag));
		req.end();
		return req;
	},

	getRequestOpts: 	function(tag) {
		var opts = {
			hostname: 	"stream.twitter.com",
			path: 	"/1.1/statuses/filter.json",
			method: "POST",
			headers: 	{
							"Authorization": 	generateOAuthHeader(this.keys,0,0,{track: tag}),
